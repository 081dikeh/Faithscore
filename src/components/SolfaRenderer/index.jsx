...
import { useSolfaStore, slashPositions } from "../../store/solfaStore";
...
function beatWidth(beat) {
  if (!beat || !beat.subdivision) return NOTE_W * 4; // default
  let w = 0;
  for (let si = 0; si < beat.subdivision; si++)
    w += slotWidth(si, beat.subdivision);
  return w;
}

// Total raw width of one measure (all beats + separators + padding)
function measureRawW(measure, slashSet) {
  if (!measure || !measure.beats) return PAD * 2 + NOTE_W * 4 + COLON_W * 3;
...
function SlotCell({
  ...
  return (
    <g onClick={onSelect} style={{ cursor: "pointer" }}>
      ...
      {isRest && <circle cx={cx} cy={y - 3} r={1.5} fill={C.lyricRul} />}
...
function MeasureRow({
  ...
  const rawW = measureRawW(measure, slashSet);
    ...
    beat.slots.forEach((slot, si) => {
      ...
      cells.push(
        <SlotCell
          ...
          onLyricClick={() =>
            onLyricClick(part.id, measureIdx, bi, si, slot.lyric || "")
          }
        />,
      );
      x += preW + noteW + sufW;
        ...
        if (col % 2 !== 0)
          elems.push(
            <rect
              ...
              x={colXs[ci]}
              y={rowY - NOTE_SZ - 3}
              width={rawMWs[col] * lineScale}
              height={VOICE_H - VOICE_G}
              fill={C.mBgAlt}
            />,
          );
      });
        ...
        elems.push(
          <MeasureRow
            ...
            onLyricClick={(partId, mIdx, bi, si, current) => {
              ...
              if (beat) {
                for (let s2 = 0; s2 < si; s2++)
                  lx += slotWidth(s2, beat.subdivision) * sc2;
      ...
      if (pIdx < parts.length - 1) {
        ...
        elems.push(
          <line
            ...
            y1={sepY}
            x2={
              leftEdge + lineCols.reduce((s, c) => s + rawMWs[c] * lineScale, 0)
            }
            y2={sepY}
            stroke={C.voiceSep}
            strokeWidth={0.6}
